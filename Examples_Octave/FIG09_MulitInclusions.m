...
nx          = 901;              ny          = 2/3*(nx-1)+1;         % Numerical resolution
Lx          = 3.0;              Ly          = Lx*(ny-1)/(nx-1);     % Model dimension
...
% Rectangles
Rectangles = [ 
    -0.2 , -0.4 , 0.3 , 0.3 ,   0/180*pi() , 0.01 ;
     1.2 ,  0.7 , 0.2 , 0.2 , 135/180*pi() , 0.01 ;
    -0.4 ,  0.4 , 0.2 , 0.2 ,  30/180*pi() , 0.01 ;
    -0.05,  0.65, 0.2 , 0.2 ,  30/180*pi() , 0.01 ;
    ];
for i = 1:floor(size(Rectangles,1))
    x0  = Rectangles(i,1);
    y0  = Rectangles(i,2);
    a   = Rectangles(i,3);
    b   = Rectangles(i,4);
    ...
    R = [ cos(phi) , -sin(phi) ;
          sin(phi) ,  cos(phi) ];
    s = [x0,y0]'; % shift to and from center
    A = (R*(A - s) + s);
    B = (R*(B - s) + s);
    C = (R*(C - s) + s);
    D = (R*(D - s) + s);
    inside = inpolygon(X,Y, [A(1),B(1),C(1),D(1)], [A(2),B(2),C(2),D(2)]);
    ETA(inside) = eta_B*Rectangles(i,6);
end
% Ellipses
Ellipses = [
    -0.8 ,  0.7 , 0.3 , 0.06 ,   0/180*pi() , 100 ;
    -1.05,  0.1 , 0.3 , 0.06 ,  30/180*pi() , 100 ;
    -0.1 , -0.3 , 0.3 , 0.06 ,  45/180*pi() , 100 ;
     1.2 , -0.4 , 0.3 , 0.06 ,  60/180*pi() , 100 ;
     0.5 , -0.5 , 0.3 , 0.06 ,  90/180*pi() , 100 ;
    -0.8 , -0.5 , 0.3 , 0.06 , 120/180*pi() , 100 ;
    -1.1 , -0.6 , 0.3 , 0.06 , 120/180*pi() , 100 ;
     0.8 ,  0.5 , 0.3 , 0.06 , 150/180*pi() , 100 ;
     0.2 ,  0.2 , 0.1 , 0.10 ,   0/180*pi() , 100 ;
    ];
for i = 1:size(Ellipses,1)
    ...
    b   = Ellipses(i,4);
    phi = Ellipses(i,5);
    inside      = (((X-x0)*cos(phi)+(Y-y0)*sin(phi)).^2/a^2 + ((X-x0)*sin(phi)-(Y-y0)*cos(phi)).^2/b^2 < 1);
    ETA(inside) = eta_B*Ellipses(i,6);
end
etamin      = 0.1*min(ETA(:));
for smo=1:1; Iix  = [2:nx-1]; Iiy  = [2:ny-1];                      % Smoothing of the initial viscosity field
